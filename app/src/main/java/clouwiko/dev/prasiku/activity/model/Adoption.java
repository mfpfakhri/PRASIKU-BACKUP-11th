package clouwiko.dev.prasiku.activity.model;

/**
 * Created by muham on 19/04/2018.
 */

public class Adoption {
    private String adoptionDeleteStatus;
    private String adoptionId;
    private String adoptionCatId;
    private String adoptionOwnerId;
    private String adoptionApplicantId;
    private String adoptionApplicantIdDeleteStatus;
    private String adoptionApplicantPhone;
    private String adoptionApplicantAddress;
    private String adoptionApplicantJob;
    private String adoptionApplicantReason;
    private String adoptionApplicantNoAnimal;
    private String adoptionApplicantHouseType;
    private String adoptionApplicantHouseSize;
    private String adoptionApplicantNoPeople;
    private String adoptionApplicantCatPlace;
    private String adoptionApplicantFamPermission;
    private String adoptionApplicantMove;
    private String adoptionApplicantMarriage;
    private String adoptionApplicantKids;
    private String adoptionApplicantFinancial;
    private String adoptionApplicationStatus;
    private String adoptionCatName;
    private String adoptionCatPhoto;
    private String adoptionApplicantName;
    private String adoptionOwnerIdApponStatus;
    private String adoptionCatIdApponStatus;
//    private String adoptionOwnerDeleteStatus;

    public Adoption() {
    }

    public Adoption(String adoptionId, String adoptionCatId, String adoptionOwnerId, String adoptionApplicantId, String adoptionApplicantIdDeleteStatus, String adoptionApplicantPhone, String adoptionApplicantAddress, String adoptionApplicantJob, String adoptionApplicantReason, String adoptionApplicantNoAnimal, String adoptionApplicantHouseType, String adoptionApplicantHouseSize, String adoptionApplicantNoPeople, String adoptionApplicantCatPlace, String adoptionApplicantFamPermission, String adoptionApplicantMove, String adoptionApplicantMarriage, String adoptionApplicantKids, String adoptionApplicantFinancial, String adoptionApplicationStatus, String adoptionCatName, String adoptionCatPhoto, String adoptionApplicantName, String adoptionOwnerIdApponStatus, String adoptionDeleteStatus, String adoptionCatIdApponStatus) {
        this.adoptionId = adoptionId;
        this.adoptionCatId = adoptionCatId;
        this.adoptionOwnerId = adoptionOwnerId;
        this.adoptionApplicantId = adoptionApplicantId;
        this.adoptionApplicantIdDeleteStatus = adoptionApplicantIdDeleteStatus;
        this.adoptionApplicantPhone = adoptionApplicantPhone;
        this.adoptionApplicantAddress = adoptionApplicantAddress;
        this.adoptionApplicantJob = adoptionApplicantJob;
        this.adoptionApplicantReason = adoptionApplicantReason;
        this.adoptionApplicantNoAnimal = adoptionApplicantNoAnimal;
        this.adoptionApplicantHouseType = adoptionApplicantHouseType;
        this.adoptionApplicantHouseSize = adoptionApplicantHouseSize;
        this.adoptionApplicantNoPeople = adoptionApplicantNoPeople;
        this.adoptionApplicantCatPlace = adoptionApplicantCatPlace;
        this.adoptionApplicantFamPermission = adoptionApplicantFamPermission;
        this.adoptionApplicantMove = adoptionApplicantMove;
        this.adoptionApplicantMarriage = adoptionApplicantMarriage;
        this.adoptionApplicantKids = adoptionApplicantKids;
        this.adoptionApplicantFinancial = adoptionApplicantFinancial;
        this.adoptionApplicationStatus = adoptionApplicationStatus;
        this.adoptionCatName = adoptionCatName;
        this.adoptionCatPhoto = adoptionCatPhoto;
        this.adoptionApplicantName = adoptionApplicantName;
        this.adoptionOwnerIdApponStatus = adoptionOwnerIdApponStatus;
        this.adoptionDeleteStatus = adoptionDeleteStatus;
        this.adoptionCatIdApponStatus = adoptionCatIdApponStatus;
    }

    public String getAdoptionId() {
        return adoptionId;
    }

    public void setAdoptionId(String adoptionId) {
        this.adoptionId = adoptionId;
    }

    public String getAdoptionCatId() {
        return adoptionCatId;
    }

    public void setAdoptionCatId(String adoptionCatId) {
        this.adoptionCatId = adoptionCatId;
    }

    public String getAdoptionOwnerId() {
        return adoptionOwnerId;
    }

    public void setAdoptionOwnerId(String adoptionOwnerId) {
        this.adoptionOwnerId = adoptionOwnerId;
    }

    public String getAdoptionApplicantId() {
        return adoptionApplicantId;
    }

    public void setAdoptionApplicantId(String adoptionApplicantId) {
        this.adoptionApplicantId = adoptionApplicantId;
    }

    public String getAdoptionApplicantIdDeleteStatus() {
        return adoptionApplicantIdDeleteStatus;
    }

    public void setAdoptionApplicantIdDeleteStatus(String adoptionApplicantIdDeleteStatus) {
        this.adoptionApplicantIdDeleteStatus = adoptionApplicantIdDeleteStatus;
    }

    public String getAdoptionApplicantPhone() {
        return adoptionApplicantPhone;
    }

    public void setAdoptionApplicantPhone(String adoptionApplicantPhone) {
        this.adoptionApplicantPhone = adoptionApplicantPhone;
    }

    public String getAdoptionApplicantAddress() {
        return adoptionApplicantAddress;
    }

    public void setAdoptionApplicantAddress(String adoptionApplicantAddress) {
        this.adoptionApplicantAddress = adoptionApplicantAddress;
    }

    public String getAdoptionApplicantJob() {
        return adoptionApplicantJob;
    }

    public void setAdoptionApplicantJob(String adoptionApplicantJob) {
        this.adoptionApplicantJob = adoptionApplicantJob;
    }

    public String getAdoptionApplicantReason() {
        return adoptionApplicantReason;
    }

    public void setAdoptionApplicantReason(String adoptionApplicantReason) {
        this.adoptionApplicantReason = adoptionApplicantReason;
    }

    public String getAdoptionApplicantNoAnimal() {
        return adoptionApplicantNoAnimal;
    }

    public void setAdoptionApplicantNoAnimal(String adoptionApplicantNoAnimal) {
        this.adoptionApplicantNoAnimal = adoptionApplicantNoAnimal;
    }

    public String getAdoptionApplicantHouseType() {
        return adoptionApplicantHouseType;
    }

    public void setAdoptionApplicantHouseType(String adoptionApplicantHouseType) {
        this.adoptionApplicantHouseType = adoptionApplicantHouseType;
    }

    public String getAdoptionApplicantHouseSize() {
        return adoptionApplicantHouseSize;
    }

    public void setAdoptionApplicantHouseSize(String adoptionApplicantHouseSize) {
        this.adoptionApplicantHouseSize = adoptionApplicantHouseSize;
    }

    public String getAdoptionApplicantNoPeople() {
        return adoptionApplicantNoPeople;
    }

    public void setAdoptionApplicantNoPeople(String adoptionApplicantNoPeople) {
        this.adoptionApplicantNoPeople = adoptionApplicantNoPeople;
    }

    public String getAdoptionApplicantCatPlace() {
        return adoptionApplicantCatPlace;
    }

    public void setAdoptionApplicantCatPlace(String adoptionApplicantCatPlace) {
        this.adoptionApplicantCatPlace = adoptionApplicantCatPlace;
    }

    public String getAdoptionApplicantFamPermission() {
        return adoptionApplicantFamPermission;
    }

    public void setAdoptionApplicantFamPermission(String adoptionApplicantFamPermission) {
        this.adoptionApplicantFamPermission = adoptionApplicantFamPermission;
    }

    public String getAdoptionApplicantMove() {
        return adoptionApplicantMove;
    }

    public void setAdoptionApplicantMove(String adoptionApplicantMove) {
        this.adoptionApplicantMove = adoptionApplicantMove;
    }

    public String getAdoptionApplicantMarriage() {
        return adoptionApplicantMarriage;
    }

    public void setAdoptionApplicantMarriage(String adoptionApplicantMarriage) {
        this.adoptionApplicantMarriage = adoptionApplicantMarriage;
    }

    public String getAdoptionApplicantKids() {
        return adoptionApplicantKids;
    }

    public void setAdoptionApplicantKids(String adoptionApplicantKids) {
        this.adoptionApplicantKids = adoptionApplicantKids;
    }

    public String getAdoptionApplicantFinancial() {
        return adoptionApplicantFinancial;
    }

    public void setAdoptionApplicantFinancial(String adoptionApplicantFinancial) {
        this.adoptionApplicantFinancial = adoptionApplicantFinancial;
    }

    public String getAdoptionApplicationStatus() {
        return adoptionApplicationStatus;
    }

    public void setAdoptionApplicationStatus(String adoptionApplicationStatus) {
        this.adoptionApplicationStatus = adoptionApplicationStatus;
    }

    public String getAdoptionCatName() {
        return adoptionCatName;
    }

    public void setAdoptionCatName(String adoptionCatName) {
        this.adoptionCatName = adoptionCatName;
    }

    public String getAdoptionCatPhoto() {
        return adoptionCatPhoto;
    }

    public void setAdoptionCatPhoto(String adoptionCatPhoto) {
        this.adoptionCatPhoto = adoptionCatPhoto;
    }

    public String getAdoptionApplicantName() {
        return adoptionApplicantName;
    }

    public void setAdoptionApplicantName(String adoptionApplicantName) {
        this.adoptionApplicantName = adoptionApplicantName;
    }

    public String getAdoptionOwnerIdApponStatus() {
        return adoptionOwnerIdApponStatus;
    }

    public void setAdoptionOwnerIdApponStatus(String adoptionOwnerIdApponStatus) {
        this.adoptionOwnerIdApponStatus = adoptionOwnerIdApponStatus;
    }

    public String getAdoptionDeleteStatus() {
        return adoptionDeleteStatus;
    }

    public void setAdoptionDeleteStatus(String adoptionDeleteStatus) {
        this.adoptionDeleteStatus = adoptionDeleteStatus;
    }

    public String getAdoptionCatIdApponStatus() {
        return adoptionCatIdApponStatus;
    }

    public void setAdoptionCatIdApponStatus(String adoptionCatIdApponStatus) {
        this.adoptionCatIdApponStatus = adoptionCatIdApponStatus;
    }
}
